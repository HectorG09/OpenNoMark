...
ChatGPT infographics have no localized mark; their defect is an 8-16px blotch texture inside flat fills, strongest beside dark text. `--mode stains` / `mode=stains` skips the localizer entirely (OWLv2 erased the headline of the reference sample as a "watermark") and runs a deterministic cleaner: structure mask from Lab gradients, connected flat regions, each joined with the nearby fragments of the same fill that text split off (line gaps closed by descenders, letter counters; `GROUP_TOLERANCE`, `GROUP_RADIUS`), a robust masked-Gaussian field per group and a soft blend toward it. JPEG ringing beside glyphs counts as structure, so the most visible blotches sit in a 16px text band: there the field of the nearest large group (`ANCHOR_AREA`) on the pixel's side of any hard edge (`HARD_EDGE`: badge rims, box outlines; inside a small badge the white page is nearer than the badge's own fill) is extended by normalised convolution (copying the single nearest pixel drew streaks and stepped patches), and each pixel is projected onto a two-colour ink/background model; only mostly-background pixels (`MAX_INK_ALPHA`) are rebuilt, and pixels lighter than the fill count as stain. Lab is computed in floating point on OpenCV's 8-bit scale and untouched pixels are copied bit for bit: an 8-bit Lab round trip alone moved half the pixels of the reference JPEG by up to 15 levels. Only near-constant fills (`FILL_RANGE`) anchor the background, and an image whose large fills cover less than `MIN_FILL_SHARE` of it is returned untouched: without that guard, portraits in `examples/` lost cloud texture and hair detail. Every pixel constant is calibrated on ChatGPT's 1024-1536px output, so the guard measures larger images on a copy scaled to `GUARD_SIZE`: at native 2400px two Gemini photos crossed the threshold. `tests/test_stain_cleaner.py` runs real `examples/` photos through the cleaner and requires them unchanged. Small groups whose colour differs from their surroundings (`DISTINCT_FILL`, e.g. green "SI" badges) anchor themselves; isolated gaps between glyphs do not. LaMa over a stain mask was measured and rejected: ~140s on CPU for a 1024x1536 sample and new speckles beside glyphs. Validation re-measures stain energy on the cleaned regions (`MAX_RESIDUAL_RATIO`), mirroring the residual check of the watermark path.
